...
const PlaceholderSchema = new mongoose.Schema({
  ...
  type: { type: String, enum: ['text', 'image', 'textarea'], required: true },
  ...
  lineHeight: { type: Number, default: 1.2 }, // Line height for textarea
  maxLines: { type: Number, default: 5 }, // Maximum lines for textarea
  isGlobal: { type: Boolean, default: false }, // Global placeholder appears on all pages
  pageOverrides: { type: Map, of: { x: Number, y: Number, width: Number, height: Number }, default: {} } // Position overrides for specific pages
});
...
TemplateSchema.methods.addPlaceholderToPage = function(placeholder, pageIndex = 0) {
  if (!['text', 'image', 'textarea'].includes(placeholder.type)) {
    throw new Error(`Invalid placeholder type: ${placeholder.type}`);
  }
